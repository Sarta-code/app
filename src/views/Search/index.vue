    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x"
                v-show="searchParams.categoryName">{{searchParams.categoryName}}
              <i v-on:click="moveCategoryName">×</i>
            </li>
            <li class="with-x"
                v-show="searchParams.keyword">{{searchParams.keyword}}
              <i v-on:click="moveKeyWord">×</i>
            </li>
            <li class="with-x"
                v-show="searchParams.trademark">{{searchParams.trademark.split(":")[1]}}
              <i v-on:click="moveTradeMark">×</i>
            </li>
            <li class="with-x"
                v-for="(props,index) of searchParams.props"
                :key="index">{{props.split(":")[1]}}
              <i v-on:click="moveProps(index)">×</i>
            </li>
          </ul>
        </div>
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}"
                    v-on:click="comprehensive">
                  <a>综合<span class="iconfont"
                          :class="{'icon-arrowTop':isUp && isOne,'icon-arrowBottom':isDown && isOne}"></span></a>
                </li>
                <li :class="{active:isTwo}"
                    v-on:click="priceSorted">
                  <a>价格<span class="iconfont"
                          :class="{'icon-arrowTop':isUp && isTwo,'icon-arrowBottom':isDown && isTwo}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
    ...
    methods: {
        ...
        priceSorted() {
            if (this.priceSortedflag) {
                this.searchParams.order = '2:asc';
                this.priceSortedflag = false;
                this.getSearchData();
            } else {
                this.searchParams.order = '2:desc';
                this.priceSortedflag = true;
                this.getSearchData();
            }
        },
        getTotle(totle) {
            this.searchParams.pageNo = totle;
            this.getSearchData();
        },
    },
    computed: {
        ...
        ...mapState({
            totle: store => {
                return store.search.searchList.total;
            },
        }),
